...
const DemoAgreementModal = ({ forceOpen = false, onClose }) => {
    ...
    // Keep modal state updated when demoMode or acceptance changes
    useEffect(() => {
        const isAcked = localStorage.getItem('digioptics_demo_acknowledged') === 'true';
        setAccepted(isAcked);

        if (isDemoMode && !isExpired && (forceOpen || !isAcked)) {
            setModalOpen(true);
        } else if (!forceOpen) {
            setModalOpen(false);
        }
    ...
    const [orgName, setOrgName] = useState(
        tenant?.storeInformation?.storeName || tenant?.companyName || tenant?.name || tenant?.organizationName || user?.companyName || 'Partner Organisation'
    );
    ...
    // Single-click handleAccept function
    ...
    if (!modalOpen || !isDemoMode || isExpired) return null;

    return (
        <div 
            className="fixed inset-0 z-[99999] flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm animate-fadeIn overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
        >
            <div 
                className="relative w-full max-w-3xl my-auto bg-white rounded-3xl shadow-2xl border border-blue-100 overflow-hidden flex flex-col max-h-[92vh]"
                onClick={(e) => e.stopPropagation()}
            >
                ...
                <div className="p-5 sm:p-7 overflow-y-auto flex-1 space-y-6 text-gray-800 text-xs sm:text-sm bg-white">
                    ...
                    <div className="p-4 sm:p-5 bg-gradient-to-br from-blue-50/80 via-white to-blue-50/50 rounded-2xl border border-blue-200 shadow-xs relative">
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <button
                                ...
                                className="text-[10px] font-extrabold uppercase tracking-wider text-erp-accent hover:underline flex items-center gap-1 bg-white/80 px-2 py-1 rounded-lg border border-blue-200 cursor-pointer"
                            >
                                <Icon icon={isEditingData ? "mdi:check" : "mdi:pencil-outline"} />
                                {isEditingData ? "Done Editing" : "Edit Details"}
                            </button>
                        </div>

                        {isEditingData ? (
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4 p-3 bg-white rounded-xl border border-blue-200 shadow-xs">
                                <div>
                                ...
                                <div>
                                    <label className="text-[10px] font-bold text-gray-500 uppercase block mb-1">Organisation</label>
                                    <input
                                        type="text"
                                        value={orgName}
                    ...
                    {/* Agreed and Accepted Receipt Card */}
